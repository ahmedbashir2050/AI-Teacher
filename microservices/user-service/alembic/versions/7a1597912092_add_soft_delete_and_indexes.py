...
def upgrade() -> None:
    ...
    op.add_column(
        "user_profiles", sa.Column("is_deleted", sa.DateTime(), nullable=True)
    )
    op.add_column("books", sa.Column("is_deleted", sa.DateTime(), nullable=True))

    op.create_index(
        op.f("ix_user_profiles_faculty_id"),
        "user_profiles",
        ["faculty_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_user_profiles_department_id"),
        "user_profiles",
        ["department_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_user_profiles_semester_id"),
        "user_profiles",
        ["semester_id"],
        unique=False,
    )
