...
async def ingest_document(
    request: Request, file: UploadFile = File(...), current_user: dict = ADMIN_ACCESS
):
    """
    ...
    try:
        ...
        return {
            "message": f"Document '{file.filename}' received and is being processed in the background."
        }

    except Exception as e:
        print(f"Ingestion submission error: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to queue the document for processing."
        )
