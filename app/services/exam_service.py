...
from app.repository import book_repository, exam_repository
...
def generate_and_store_exam(db: Session, exam_request: ExamRequest):
    ...
    relevant_chunks = retrieve_relevant_chunks(
        f"Exam material for {exam_request.chapter}",
        top_k=15,
        collection_name=book.qdrant_collection_name,
    )
    ...
    db_exam = exam_repository.create_exam(
        db, f"Final Exam: {exam_request.chapter}", book
    )
