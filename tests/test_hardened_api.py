...
def test_login_for_access_token():
    response = client.post(
        "/api/auth/token", data={"username": "student", "password": "studentpassword"}
    )
...
def test_login_invalid_credentials():
    response = client.post(
        "/api/auth/token", data={"username": "student", "password": "wrongpassword"}
    )
...
def test_register_user():
    response = client.post(
        "/api/auth/register", json={"username": "newuser", "password": "newpassword"}
    )
    ...
    response = client.post(
        "/api/auth/token", data={"username": "newuser", "password": "newpassword"}
    )
    assert response.status_code == 200
...
def test_chat_student_access(student_token):
    headers = {"Authorization": f"Bearer {student_token}"}
    response = client.post(
        "/api/chat", json={"question": "What is economics?"}, headers=headers
    )
    assert response.status_code == 200
